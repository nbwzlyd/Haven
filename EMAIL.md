...
- ⏭ **Next:** CP-6 (SMTP send: `MailClient.send`, Proton stays 501) + CP-7 (full-screen compose UI). Outstanding: positive IMAP-over-a-*working*-tunnel is unproven (needs a remote IMAP reachable only via a live tunnel — infra we don't have on one device); optional in-process GreenMail CI test for the mapping; 11-locale translation of the CP-4 strings.
