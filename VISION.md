...
- **Tier 3 — standing policy.** A small deterministic program the agent authors and the user installs once, against a declared scope (exactly which tools, which targets), a rate ceiling, and an expiry. A kill-switch lives on the agent-activity chip; every tick is still written to the audit table. *(Forward-looking — the shipped "allow N min" window and paired-client bypass are the seed; scope-binding, rate ceilings, expiry, and the kill-switch are not built.)*
...
What is missing is symmetry: the loop can act on its body but cannot yet *perceive* it. There is no resource that returns Haven's own rendered screen, and no verb that drives Haven's own UI — terminal input and predefined navigation exist, but general UI input injection does not. Closing that gap — a screen-as-resource plus a UI input verb, both governed by the consent tiers above and recorded to the audit log — is what turns the human-in-the-loop release-verify into a self-hosted one. Until then the loop runs under Tiers 1–2 with a human eye on the screen, and any Tier-4 act — replacing the running app, editing the constitution — is surfaced for a human's tap. The agent↔agent half of the loop is prototyped in `docs/haven_convo_prototype.py`.
...
3. **Consolidate the agent-presence layer** — lift the reach-back tools (`present_media`/`present_app`/`raise_notification`/`queue_self_message`/consent) into one documented bidirectional model, and add MCP `resources/*` (§1a).
...
- **MCP `resources/*` capability** — file-shaped resources for "what's on the screen right now" so an agent can pull a snapshot without polling. Two pieces close the self-hosting perception gap (see "The self-hosting loop"): a **resource that returns Haven's own rendered UI** (today only terminal and remote-desktop snapshots exist) and a **verb that drives Haven's own UI** (today only terminal input + predefined navigation exist).
- **Tier-3 standing policies** — let an agent install a scoped, rate-capped, expiring reflex once instead of one consent sheet per tick (see "Consent tiers"), with a kill-switch on the agent-activity chip and every tick still audited. The shipped "allow N min" window and paired-client bypass are the seed.
