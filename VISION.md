...
- **GL version ceiling and forward options (verified on a Pixel 8 Pro).** The cage GL broker shipped (v5.59.63): app-level OpenGL in a Sway cage now runs on Mali (`GL_RENDERER = virgl (Mali-G715)`) instead of llvmpipe. But it is capped at **GL 2.1**, and this is a hard ceiling for *desktop* GL: Android's EGL exposes only **GLES**, and virgl's GLES→desktop-GL translation caps the *compatibility* profile at 2.1 — so legacy/desktop-GL apps (glxgears, older games, most CAD viewers) cannot exceed it. A **dead-end was verified**: bumping the host virgl EGL context from ES2→ES3 (`vrend_winsys_egl.c`, rebuilt + device-tested) did **not** raise the desktop-GL version (glxgears stayed 2.1); a higher GLES context can only lift *GLES-native* caps, which the software-Xwayland cage couldn't even measure (`glxinfo` → `X_GetImage BadMatch`, `eglinfo` hangs). Forward options in payoff order: **(1) venus + guest zink — spiked, partially works, currently blocked.** Built a venus-enabled `virgl_test_server` + self-locating `virgl_render_server` (NDK cross-build, `-Dvenus=true`, render-server path via `/proc/self/exe`), `apt install mesa-vulkan-drivers` provides the guest venus ICD (`libvulkan_virtio.so` + `virtio_icd.json`). **Device-verified the transport works**: `vulkaninfo` in the cage reports a real Vulkan device `Virtio-GPU Venus (Mali-G715)`, `DRIVER_ID_MESA_VENUS`, apiVersion 1.4 — i.e. guest Vulkan → vtest socket → host venus → Android `libvulkan` → Mali, **with no `/dev/dri`/virtio-gpu kernel device**. **The allocation wall was found, fixed, and cleared — then a deeper blocker surfaced.** First root cause (debug render server + captured stderr): `vkr: minigbm_allocation is not enabled` → `vkBindBufferMemory2 CS error` → a FORTIFY abort — virglrenderer's venus (`src/venus/vkr_device_memory.c`) allocates exportable device memory only via **minigbm**, which needs a `/dev/dri` render node the sandbox lacks. **Fix built + device-verified:** an **AHardwareBuffer allocator** was added to vkr's device-memory path (`#elif defined(ENABLE_AHB_ALLOCATION)` — AHB `BLOB` → `getNativeHandle` → dup'd dma-buf fd → `VkImportMemoryFdInfoKHR{DMA_BUF_BIT_EXT}`; the host Mali Vulkan was confirmed to advertise `VK_EXT_external_memory_dma_buf`, so no `/dev/dri` needed). **This works** — memory now allocates + binds, the abort is gone, and the pipeline advances to the host processing the **full venus command stream** (instrumented: 308 commands consumed cleanly). **Coherency was ruled out** as the next suspect: the host ring monitor reads the guest's tail and processes everything over the shared AHB memory — expected, since guest+host are CPUs on one ARM SoC (two cached dma-buf mmaps are HW-coherent; the ring's C11 atomics rely on that). **Current blocker = venus GPU-fence completion:** the host consumes the whole stream and idles, but the fence/timeline completion never reaches the guest, so the guest spins in `MESA-VIRTIO: stuck in fence wait → aborting`. vtest *does* wire the venus fence path (`vtest_renderer.c vtest_write_context_fence` → `vtest_signal_timeline`, `.write_context_fence`); the open question is whether that callback fires (host GPU submit completing the fence) or the timeline→guest signal is lost across the render-server/proxy boundary (`render-server-worker=thread`). This is deep venus async-fence territory and may be a **fundamental vtest+venus gap**, not a quick fix — **paused here**. Net: the AHB allocator moved venus-in-the-cage from *"can't allocate memory"* to *"processes the full GL command stream but can't complete a fence"*. (Mali also misses two zink base feats `fillModeNonSolid`/`shaderClipDistance`, a separate, lesser issue.) **Status: the AHB allocator + venus build patches are uncommitted working-tree changes; not shipped.** **(2) GLES-renderer cage compositor** — `WLR_RENDERER=gles2` wlroots over virgl, so the *compositor* (not just the app) is GPU-accelerated (full-pipeline). **(3) Xwayland in the cage by default** — shipped, so X11/GLX apps run at all. Note the broker is GLES-only today; it does **not** unblock the GLES-only nested compositors (Hyprland/aquamarine, niri/smithay still fail `CBackend::create()` — they need a DRM render node, not a GL client driver).
